...
class NoticeApiIntegrationTest extends AbstractIntegrationTest {
    ...
    void create_validRequest_returns200WithCreatedData() throws Exception {
        mockMvc.perform(post(BASE_URL)
            ...
            .andExpect(jsonPath("$.title").value("test-value"))
            .andExpect(jsonPath("$.content").value("test-value"))
            .andExpect(jsonPath("$.authorId").value(100))
            .andExpect(jsonPath("$.hit").value(100));
    }
    ...
    void update_existingId_returns200WithUpdatedData() throws Exception {
        ...
        mockMvc.perform(put(BASE_URL + "/" + id)
            ...
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(id))
            .andExpect(jsonPath("$.title").value("updated-value"))
            .andExpect(jsonPath("$.content").value("updated-value"))
            .andExpect(jsonPath("$.authorId").value(200))
            .andExpect(jsonPath("$.hit").value(200));
    }

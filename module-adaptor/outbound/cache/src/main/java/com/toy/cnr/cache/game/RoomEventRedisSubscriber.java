...
/**
 * Redis Pub/Sub 기반 방 이벤트 구독 구현.
 * <p>
 * 동일 방 채널에 복수의 클라이언트(SSE 커넥션)가 독립적으로 구독할 수 있도록
 * UUID 기반 {@code subscriberId}로 각 구독을 관리합니다.
 * <p>
 * Channel: {@code room:{roomId}:events}
 */
@Component
public class RoomEventRedisSubscriber implements RoomEventSubscriber {

    private final RedisMessageListenerContainer listenerContainer;
    private final ObjectMapper objectMapper;
    private final Map<String, MessageListener> activeListeners = new ConcurrentHashMap<>();

    public RoomEventRedisSubscriber(
    ...
    @Override
    public String subscribe(String roomId, Consumer<RoomEventDto> onMessage) {
        var subscriberId = GameKey.generateRoomSubscriberId(roomId);
        var topic = new ChannelTopic(GameKey.roomEvents(roomId));

        MessageListener listener = (message, pattern) -> {
            try {
                var dto = objectMapper.readValue(message.getBody(), RoomEventDto.class);
